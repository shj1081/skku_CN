...
def calculate_expression(expr_str: str) -> float:
    ...
    while re.search(sqrt_pattern, expr):
        ...
        try:
            num = float(num_str)
            if num < 0:
                raise ValueError(f"Cannot calculate square root of a negative number: {num}")
    ...
    for i in range(len(tokens) - 1):
        if isinstance(tokens[i], str) and isinstance(tokens[i+1], str) and tokens[i+1] != '-':
    ...
    def calculate(tokens_list):
        ...
        while i < len(tokens_list):
            ...
            if tokens_list[i] == '*':
                if i == 0 or i == len(tokens_list) - 1 or isinstance(tokens_list[i-1], str) or isinstance(tokens_list[i+1], str):
...
def handle_client(conn, addr):
    ...
    try:
        conn.sendall(b"Welcome to TCP Calculator Server!\nEnter expression or GET filename:\n")
        while True:
            data = conn.recv(1024)
            if not data:
                break
